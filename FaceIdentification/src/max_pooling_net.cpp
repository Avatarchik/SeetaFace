...
void MaxPoolingNet::Execute() {
  ...
  //float* const dst_head = new float[dst_count]; //lx-avoid data copy
...
#pragma omp for nowait
    ...
    for (int c = 0; c < channels; ++c) {
	  const float* psrc_data = src_data + c*src_channel_off;
	  float* pdst_data = dst_data + c*dst_channel_off;
        ...
        for (int dw = 0, wstart = 0; dw < dst_w;
          ...
          for (int sh = hstart; sh < hend; ++sh) {
            for (int sw = wstart; sw < wend; ++sw) {
              ...
              if (psrc_data[sidx] > max_val) {
                max_val = psrc_data[sidx];
              }
            } // for sw
          } // for sh
          pdst_data[didx] = max_val;
        } // for dw
      } // for dh
    } // for c
#ifdef USE_OPENMP
  ...
  //output->CopyData(num, channels, dst_h, dst_w, dst_head); //lx-avoid data copy
  //delete[] dst_head;
  CheckOutput();
}
