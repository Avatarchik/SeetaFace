...
void EltwiseNet::Execute() {
  ...
  if (op_ == "BAIS_ADDER") {
    ...
    //float* const dst_head = new float[num*channels*height*width]; //lx-avoid data copy
	output->SetData(num,channels,height,width);
...
#pragma omp for nowait
      ...
      for (int c = 0; c < channels; ++ c) {
		int offset = (n*channels+c)*width*height;
    ...
    //output->CopyData(num, channels, height, width, dst_head);  //lx-avoid data copy 
	//delete[] dst_head;
  }
  else if (op_ == "SCALE") {
    const Blob* const input = this->input_blobs(0);
