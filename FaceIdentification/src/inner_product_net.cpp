...
void InnerProductNet::Execute() {
  ...
  //float* const dst_head = new float[src_num * dst_channels]; //lx-avoid data copy
  output->SetData(src_num,dst_channels,1,1);
...
#pragma omp for nowait
		...
		for (int dc = 0; dc < dst_channels; ++dc) {
			int didx = sn*dst_channels + dc;
		  const float* tmp_weight_data = weight_data+dc*vec_len;
		  dst_head[didx++] = simd_dot(src_data, tmp_weight_data, vec_len);
		  //weight_data += vec_len;
		} // for dc
#ifdef USE_OPENMP
	}
  ...
  //output->CopyData(src_num, dst_channels, 1, 1, dst_head); //lx-avoid data copy
  //delete[] dst_head;
