...
float simd_dot(const float* x, const float* y, const long& len) {
  ...
  float32x4_t acc = vdupq_n_f32(0); // set to (0, 0, 0, 0)
  float temp[4];
  ...
  inner_prod += vgetq_lane_f32(acc,0);
  inner_prod += vgetq_lane_f32(acc,1);
  inner_prod += vgetq_lane_f32(acc,2);
  inner_prod += vgetq_lane_f32(acc,3);
