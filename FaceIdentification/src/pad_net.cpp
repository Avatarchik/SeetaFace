...
void PadNet::Execute() {
  ...
  //float* const data = new float[num * channels * dst_h * dst_w](); //lx-avoid data copy
  output->SetDataInit(num, channels, dst_h, dst_w);
  float* const data = output->data().get();

  for (int n = 0; n < num; ++ n) {
    int src_off = input->offset(n), dst_off = output->offset(n);

#ifdef USE_OPENMP
#pragma omp parallel num_threads(SEETA_NUM_THREADS) private(src_off,dst_off)
	{
#pragma omp for nowait
	...
	for (int c = 0; c < channels; ++ c) {
		if (top_ < 0 && left_ < 0) {
			...
			dst_off = output->offset(n,c);
		}
		else if(top_ >= 0 && left_ >= 0) {
			src_off = input->offset(n, c);
  ...
  //output->CopyData(num, channels, dst_h, dst_w, data); //lx-avoid data copy
  //delete[] data;
  CheckOutput();
}
