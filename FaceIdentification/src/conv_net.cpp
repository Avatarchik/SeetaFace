...
void ConvNet::Execute() {
  ...
  //float* const dst_head = new float[src_num * dst_size * dst_channels]; //lx-avoid data copy
  output->SetData(src_num, dst_channels, dst_h, dst_w);
  float* const dst_head = output->data().get();
...
#ifdef USE_OPENMP
#pragma omp parallel num_threads(SEETA_NUM_THREADS) private(mat_data)
	{
	#pragma omp for nowait
#endif
		for (int sh = 0; sh < end_h; sh += stride_h_) {
#ifdef USE_OPENMP
		  mat_data = mat_head + (sh/stride_h_)*(end_w/stride_w_)*src_channels*kernel_h*kernel_w;
#endif
...
#endif
  //output->CopyData(src_num, dst_channels, dst_h, dst_w, dst_head);  //lx-avoid data copy
  //delete[] dst_head;
  delete[] mat_head;
